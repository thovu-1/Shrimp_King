...
class Crystal_Black(Shrimp):
    def __init__(self, x, y, level, food_bar, id = None):
        super().__init__(x, y, level, food_bar, 'Crystal_Black', DIRECTION, id)
        self.level = level
        self.next_level =  0 
    ...
    def level_up(self):
        ...
        self.get_next_level()
        if self.level <= 3:
            self.sprite_data = self.load_sprites()['Crystal_Black'][self.level]  # Update Sprite Data
    ...
    def load_sprites(self):
        d = {}
        # Load sprite sheets
        shrimplet_sprite_sheet = pygame.image.load('classes/shrimp/shrimp_images/Shrimplet_Crystal_Black.png').convert_alpha()
        juvenile_sprite_sheet = pygame.image.load('classes/shrimp/shrimp_images/Juvenile_Crystal_Black.png').convert_alpha()
        adult_sprite_sheet = pygame.image.load('classes/shrimp/shrimp_images/Adult_Crystal_Black.png').convert_alpha()

        # Load frame data from JSON files
        with open('classes/shrimp/shrimp_images/Shrimplet_Crystal_Black.json') as f:
            shrimplet_data = json.load(f)
        with open('classes/shrimp/shrimp_images/Juvenile_Crystal_Black.json') as f:
            juvenile_data = json.load(f)
        with open('classes/shrimp/shrimp_images/Adult_Crystal_Black.json') as f:
